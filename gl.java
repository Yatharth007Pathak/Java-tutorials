...
public class gl {: This line defines a public class named gl. In Java, every application must have at least one class definition.
...
for (int j = i; j < rows; j++) {: This is the first inner for loop that handles the printing of spaces before the numbers in the current row.
It starts with j equal to i and continues until j is less than rows. 
